...
const riskScoreCalculator = async (validatorsData, provider) => {
  ...
  accountsDetailsJSON.forEach((element) => {
    if(element.eraPoints.length !== undefined){
        element.activeEras = element.eraPoints.length;
    }else {
        element.activeEras = 0;
    }
...
//   console.log("+++++restructuring data+++++");
  accountsDetailsJSON.forEach((element) => {
    ...
    const slashScore =
      (1 + slashesInfo[element.stashId].slashCount) / (element.activeEras + 1);
...
//   console.log(minRS)
  riskScoreArr.forEach((x) => {
    // console.log('stashId: ' + x.stashId + ', RS: ' + x.riskScore + ', normalizedRS: ' + normalizeData(x.riskScore, maxRS, minRS)  )
